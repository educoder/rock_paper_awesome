...
class RockPaperAwesome
  ...
  define_statemachine :arduino do
    ...
    state :OFFLINE do
      on :connect, :to => :WAITING_FOR_XMPP_CONNECTION, 
        :action => proc{|rpa| output("~") }
    end

    state :WAITING_FOR_XMPP_CONNECTION do
      on :connected, :to => :WAITING_FOR_EITHER_READY,
        :if => proc{|input| input == "@"}
    end

    state :WAITING_FOR_EITHER_READY do
    ...
    state :WAITING_FOR_THEIR_READY do
      on :remote_ready, :to => :WAITING_FOR_EITHER_CHOICE,
        :if => proc{|input| input == "N"}
    end

    state :WAITING_FOR_YOUR_READY do
      on :ready, :to => :WAITING_FOR_EITHER_CHOICE, 
        :action => proc{|rpa| output("n") }
    end

    state :WAITING_FOR_EITHER_CHOICE do
    ...
    state :WAITING_FOR_RESULT do
      on :you_win, :to => :WAITING_FOR_EITHER_READY, 
        :if => proc{|input| input == "+"},
        :action => proc{|rpa| rpa.win_count += 1}
      on :you_lose, :to => :WAITING_FOR_EITHER_READY, 
        :if => proc{|input| input == "-"}, 
        :action => proc{|rpa| rpa.lose_count += 1}
      on :tie, :to => :WAITING_FOR_EITHER_READY, 
        :if => proc{|input| input == "="}
    end
  end
end
